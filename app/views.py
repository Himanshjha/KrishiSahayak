...
def upload_page(request):
    ...
    if request.method == 'POST' and 'file' in request.FILES:
        ...
        # Uncomment below when you want to enable prediction
...
def SolutionView(request):
    ...
    # Handle healthy plants differently
    if disease_name == 'Tomato___healthy':
        context = {
            ...
            'organic_solution': 'No treatment needed',
            'organic_solution_is_dict': False,
            'inorganic_solution': 'No treatment needed',
            'inorganic_solution_is_dict': False,
            'familiar_pesticides': 'None required',
            'familiar_pesticides_is_list': False,
            'prevention': disease_info.get('prevention', 'Continue good agricultural practices'),
            'additional_remedies': 'No additional remedies needed for healthy plants'
        }
    else:
        organic_sol = disease_info.get('organic_solution', {})
        inorganic_sol = disease_info.get('inorganic_solution', {})
        familiar_pests = disease_info.get('familiar_pesticides', [])
        context = {
            'disease_name': disease_name,
            ...
            'organic_solution': organic_sol,
            'organic_solution_is_dict': isinstance(organic_sol, dict),
            'inorganic_solution': inorganic_sol,
            'inorganic_solution_is_dict': isinstance(inorganic_sol, dict),
            'familiar_pesticides': familiar_pests,
            'familiar_pesticides_is_list': isinstance(familiar_pests, list),
            'prevention': disease_info.get('prevention', 'No prevention information available'),
            'additional_remedies': 'Consult local agricultural extension services for region-specific remedies'
        }
...
def get_plant_questions():
    ...
    return '{"questions": ["What is the plant name?", "Is it kept indoors or outdoors?", "What kind of climate is it in?", "Are there any visible problems?", "How often is it watered?"]}'
...
def get_plant_solution(answers_dict):
    ...
    # return result.content.strip()
    return '{"plant": "Unknown", "summary": "Plant care advice", "care_advice": "Water regularly", "tips": ["Tip 1", "Tip 2"]}'
